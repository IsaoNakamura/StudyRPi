...
#my %authSlack = ();
#$authSlack{"host"} = $host;
#$authSlack{"token"} = $token;
#$authSlack{"channel"} = $channel;
#if(writeJson(\\%authSlack, "./my_exec/twitterAPI/AuthSlack.json", ">")!=0){
#    print "FileSaveError. AuthSlack.\n";
#    exit -1;
#}
#exit 0;

#my %authTwitter = ();
#$authTwitter{"consumer_key"} = $consumerKey;
#$authTwitter{"consumer_secret"} = $consumerSecret;
#$authTwitter{"access_token"} = $accessToken;
#$authTwitter{"access_token_secret"} = $accessTokenSecret;
#if(writeJson(\\%authTwitter, "./AuthTwitter.json", ">")!=0){
#    print "FileSaveError. Auth.\n";
#    exit -1;
#}
#exit 0;
...
#my @keys = keys %{$authTwitter};
#my $keys_num =@keys;
#print "keys_num=$keys_num \n";
#if( exists $authTwitter->{"consumer_key"} ){
#    print $authTwitter->{"consumer_key"} . "\n";
#}
#exit 0;
