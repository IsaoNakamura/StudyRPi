        ...
        )
        {
            ...
            try
            {
                ...
                while ((strLine = stream.ReadLine())!=null)
                {
                    ...
                    DateTime cur_timestamp = DateTime.Parse(candleFactor[0]);
                    double openPrice = double.Parse(candleFactor[1].Split('=')[1]);
                    double closePrice = double.Parse(candleFactor[2].Split('=')[1]);
                    double highPrice = double.Parse(candleFactor[3].Split('=')[1]);
                    double lowPrice = double.Parse(candleFactor[4].Split('=')[1]);
                    ...
                    DateTime prev_timestamp = cur_timestamp.AddSeconds(-1.0 * periods);
                    int comple_candlenum = 1;
                    if (pastCandleBuf.getCandleCount()>0)
                    {
                        Candlestick prevCandle = pastCandleBuf.getLastCandle();
                        if (prevCandle == null)
                        {
                            Console.WriteLine("cur's candle-value 0. and prev's candle is null. timestamp=", cur_timestamp);
                            result = -1;
                            return result;
                        }

                        prev_timestamp = DateTime.Parse(prevCandle.timestamp);
                        TimeSpan prev_span = cur_timestamp - prev_timestamp;
                        double prev_elapsed_sec = prev_span.TotalSeconds;
                        comple_candlenum = (int)(prev_elapsed_sec / periods);
                    ...
                    for(int i=0; i< comple_candlenum; i++)
                    {
                        DateTime comple_timestamp = prev_timestamp.AddSeconds((i+1) * periods);
                        Candlestick candle = pastCandleBuf.addCandle(highPrice, lowPrice, openPrice, closePrice, volume, comple_timestamp.ToString(), periods, false);
                        if (candle == null)
                        {
                            Console.WriteLine("failed to addCandle.");
                            continue;
                        ...
                        Console.WriteLine("COMP: {0}, open={1}, close={2}, high={3}, low={4}, ema={5:0}", comple_timestamp.ToString(), openPrice, closePrice, highPrice, lowPrice, candle.ema);
                ...
                for (int i = 0; i < candle_addnum; i++)
                {
                    ...
                    Console.WriteLine(" ADD: {0}, open={1}, close={2}, high={3}, low={4}, ema={5:0}", add_timestamp.ToString(), candle.open, candle.last, candle.high, candle.low, candle.ema);
                }
            }
            catch (Exception ex)
            {
            ...
            try
            {
                ...
                if (retArray != null)
                {
                    ...
                    if (sideTotal == "BUY")
                    {
                        ...
                        {
                            m_position.entryLongOrder("hoge", dateTotal, amountTotal, -1, double.MaxValue);
                    ...
                    else if (sideTotal == "SELL")
                    {
                        ...
                        {
                            m_position.entryShortOrder("hoge", dateTotal, amountTotal, -1, double.MinValue);
            ...
            try
            {
                ...
                //while (true)
                {
                    ...
                    if (applyCandlestick(m_candleBuf, ohlc, m_config.periods) != 0)
                    {
                        ...
                        if (applyCandlestick(m_candleBuf, @"./btcfx_candle_1min_buf.txt", m_config.periods, true) != 0)
                        {
     ...
     //               if (m_position.isNone())
                    ...
                    if (isCloseTop == true)
                    {
                        ...
                        if (curCandleTop != null)
                        {
                            ...
                            curCandleTop.save(@"./btcfx_candle_5min_buf.txt");
                        }
                        // 次の更新時間を更新
                        nextCloseTimeTop = nextCloseTimeTop.AddSeconds(m_config.periods_top);
                    ...
                    if (isClose == true)
                    {
                        ...
                        if (curCandle != null)
                        {
                            ...
                            curCandle.save(@"./btcfx_candle_1min_buf.txt");
                        }
                        // 次の更新時間を更新
                        nextCloseTime = nextCloseTime.AddSeconds(m_config.periods);
		...
		public async Task<int> tryEntryOrder(double next_open)
        {
            ...
            try
            {
                ...
                bool isLong = false;
                bool isShort = false;
                bool isLongReserve = false;
                bool isShortReserve = false;
                bool isFibLong = false;
                bool isFibShort = false;
                int fib_long_index = -1;
                int fib_short_index = -1;
                double fib_origin_high = double.MaxValue;
                double fib_origin_low = double.MinValue;
                if (
                    calcConditionEntry
                    (
                        out isLong,
                ...
                if (m_position.isNone())
                {
                    ...
                    if (isLong || isShort || isLongReserve || isShortReserve)
                    {
                    ...
                    if ( (isLong || isLongReserve) && (curCandle.disparity_rate < disparity_border) )
                    {
                        //Console.WriteLine("Try Long Entry Order.");                  

                        if (isLong && !isLongReserve)
                        {
                            SendChildOrderResponse retObj = null;
                            int retry_cnt = 0;
                            while (true)
                            ...
                            m_position.entryLongOrder(retObj.child_order_acceptance_id, curCandle.timestamp, m_config.amount, fib_long_index, fib_origin_high);
                        ...
                        {
                            // LONG予約
							m_position.reserveLongOrder(curCandle, fib_long_index, fib_origin_high);
                            //postSlack(string.Format("{0} Long Reserved.", curCandle.timestamp));
                        }
                    }
					else if ( (isShort || isShortReserve) && (curCandle.disparity_rate > -disparity_border) )
                    {
                        //Console.WriteLine("Try Short Entry Order.");

                        if (isShort && !isShortReserve)
                        {
                            SendChildOrderResponse retObj = null;
                            int retry_cnt = 0;
                            while (true)
                            ...
                            m_position.entryShortOrder(retObj.child_order_acceptance_id, curCandle.timestamp, m_config.amount, fib_short_index, fib_origin_low);
                        ...
                        {
                            // SHORT予約
							m_position.reserveShortOrder(curCandle, fib_short_index, fib_origin_low);
                            //postSlack(string.Format("{0} Short Reserved.", curCandle.timestamp));
                        }
                    }
                }
                else
        ...
        //                    }
          ...
          //                              System.Threading.Thread.Sleep(1000);
                                ...
                                postSlack(string.Format("Long(Reserved) Entry Order ID = {0}", retObj.child_order_acceptance_id));
                            }
                        }
                        else
                        {
       ...
       //                     }
          ...
          //                              System.Threading.Thread.Sleep(1000);
                                ...
                                postSlack(string.Format("Short(Reserved) Entry Order ID = {0}", retObj.child_order_acceptance_id));
                            }
                        }
                        else
                        {
                            // SHORT予約キャンセル
                            m_position.cancelReserveOrder();
                            //postSlack(string.Format("{0} Cancel Short Reserved.", curCandle.timestamp));
                        }
                    }
                }
            }
                ...
                if (m_position.isNone())
                {
                    ...
                    if (isLong || isLongReserve)
                    {
                        if (isLong && !isLongReserve)
                        {
                            ...
                            m_position.entryLongOrder(long_id, curCandle.timestamp, m_config.amount, fib_long_index, fib_origin_high);
                            postSlack(string.Format("Long Entry Order ID = {0} ", long_id), onlyConsole);

                            long_entry_cnt++;
                        }
                        else if (!isLong && isLongReserve)
                        {
                            // LONG予約
                            m_position.reserveLongOrder(curCandle, fib_long_index, fib_origin_high);
                            //postSlack(string.Format("{0} Long Reserved.", curCandle.timestamp), onlyConsole);

                        }
                    }
                    else if (isShort || isShortReserve)
                    {
                        if (isShort && !isShortReserve)
                        {
                            // 注文成功
                            ...
                            m_position.entryShortOrder(short_id, curCandle.timestamp, m_config.amount, fib_short_index, fib_origin_low);

                            postSlack(string.Format("Short Entry Order ID = {0}", short_id), onlyConsole);
                        ...
                        else if (!isShort && isShortReserve)
                        {
                            // SHORT予約
                            m_position.reserveShortOrder(curCandle, fib_short_index, fib_origin_low);
                            //postSlack(string.Format("{0} Short Reserved", curCandle.timestamp), onlyConsole);
                        }
                    }
                }
                else
                    ...
                    if (m_position.isLongReserved())
                    {
						if (isLongReserve)
                        {
                            if (isLong)
							{
								...
								m_position.entryLongOrder(long_id, curCandle.timestamp, m_config.amount, fib_long_index, fib_origin_high);
                                // 注文成功
                                postSlack(string.Format("Long(Reserved) Entry Order ID = {0} ", long_id), onlyConsole);
                        ...
                        else
                        {
                            // LONG予約キャンセル
                            m_position.cancelReserveOrder();
                            //postSlack(string.Format("{0} Cancel Long Reserved.", curCandle.timestamp), onlyConsole);
                        }
                    }
                    else if (m_position.isShortReserved())
                    {
                        ...
                        {                     
                            if (isShort)
							{
								...
								m_position.entryShortOrder(short_id, curCandle.timestamp, m_config.amount, fib_short_index, fib_origin_low);
                                // 注文成功
                                postSlack(string.Format("Short(Reserved) Entry Order ID = {0}", short_id), onlyConsole);
                                short_entry_cnt++;
							}
                        }
                        else
                        {
                            // SHORT予約キャンセル
                            m_position.cancelReserveOrder();
                            //postSlack(string.Format("{0} Cancel Short Reserved.", curCandle.timestamp), onlyConsole);
                        }
                    }
                }
            }
        ...
        private int calcConditionEntry
        (
            out bool isLong,
            out bool isShort,
            out bool isLongReserve,
            out bool isShortReserve,
            out bool isFibLong,
            out bool isFibShort,
            out int fib_long_index,
            out int fib_short_index,
            out double fib_origin_high,
            out double fib_origin_low,
            double next_open,
        ...
        )
        {
            ...
            try
            {
                ...
                bool isCondLong = isConditionLongEntryFL(next_open);
                bool isCondShort = isConditionShortEntryFL(next_open);
                ...
                fib_origin_high = high_max;
                fib_origin_low = low_min;
                    ...
                    bool isBreakBigTrendBtm = false;
                    {
                        ...
                        if (peakMaxIndexList.Count() > 2)
                        {
                            ...
                            if (isBreakBigTrendTop)
                            {
                                ...
                                if (isCondShort && !isGolden && isBeg && (isFibLong || isCrossingSub))
                                {
                                    postSlack
                                    (
                        ...
                        if (peakMinIndexList.Count() > 2)
                        {
                            ...
                            if (isBreakBigTrendBtm)
                            {
                                // 下側トレンドラインを割った
                                // LONGは要注意
                                if (isCondLong && isGolden && isBeg && (isFibShort || isCrossingSub))
                                {
                                    postSlack
                                    (
                ...
                //bool isBreakFrontlineLong = false;
                //bool isBreakFrontlineShort = false;
                //{
                //    const int frontline_over_limit = 3;
                //    int frontlineLongOverCnt = 0;
                //    for (int i = m_candleBuf.getCandleCount() - 1; i >= 0; i--)
                //    {
                //        if (frontlineLongOverCnt >= frontline_over_limit)
                //        {
                //            isBreakFrontlineLong = true;
                //            break;
                //        }
                ...
                //            continue;
                //        }
                //        if (m_frontlineLong < candle.low)
                //        {
                //            frontlineLongOverCnt++;
                //        }
                //        else
                //        {
                //            break;
                //        }
                //    }
                //    int frontlineShortOverCnt = 0;
                //    for (int i = m_candleBuf.getCandleCount() - 1; i >= 0; i--)
                //    {
                //        if (frontlineShortOverCnt >= frontline_over_limit)
                //        {
                //            isBreakFrontlineShort = true;
                //            break;
                //        }
                //        Candlestick candle = m_candleBuf.getCandle(i);
                //        if (candle == null)
                ...
                //            continue;
                //        }
                //        if (m_frontlineShort > candle.high)
                //        {
                //            frontlineShortOverCnt++;
                //        }
                //        else
                //        {
                //            break;
                ...
                if (m_position.isNone())
                {
                    ...
                    //if (isCondLong && isGolden && !isBreakTrendBottom && !isBBHighLeak)
                    {
                        ...
                        //if (isBeg && ((isCrossingSub && isBreakTrendTop) || isFibLong ))
                        {
                            // LONG
                            isLong = true;
                            postSlack(string.Format("{0} Long Condition. isCond={1} isGold={2} bkCnt={3} isBeg={4} brkBtm={5} brkTop={6} ingEma={7} ingEmaS={8} isFib({9})={10}"
                        ...
                        if (isBeg /*&& (isCrossedSub || isCrossed)*/ )
                        {
                            // LONG予約
                            isLongReserve = true;
                            postSlack(string.Format("{0} Long Reserve Condition. isCond={1} isGold={2} bkCnt={3} isBeg={4} brkBtm={5} brkTop={6} ingEma={7} ingEmaS={8} isFib({9})={10}"
                                , curCandle.timestamp, isCondLong, isGolden, back_cnt, isBeg, isBreakTrendBottom, isBreakTrendTop, isCrossing, isCrossingSub, fib_long_index, isFibLong), onlyConsole);
                        }
                    }
                    ...
                    //else if (isCondShort && !isGolden && !isBreakTrendTop && !isBBLowLeak)
                    {
                        ...
                        //if (isBeg && ((isCrossingSub && isBreakTrendBottom) || isFibShort))
                        {
                            // SHORT
                            isShort = true;
                            postSlack(string.Format("{0} Short Condition. isCond={1} isDead={2} bkCnt={3} isBeg={4} brkBtm={5} brkTop={6} ingEma={7} ingEmaS={8} isFib({9})={10}"
                                , curCandle.timestamp, isCondShort, !isGolden, back_cnt, isBeg, isBreakTrendBottom, isBreakTrendTop, isCrossing, isCrossingSub, fib_short_index, isFibShort), onlyConsole);
                        }
                        else
                        if (isBeg /*&& (isCrossedSub || isCrossed)*/)
                        {
                            // SHORT予約
                            isShortReserve = true;
                            postSlack(string.Format("{0} Short Reserve Condition. isCond={1} isDead={2} bkCnt={3} isBeg={4} brkBtm={5} brkTop={6} ingEma={7} ingEmaS={8} isFib({9})={10}"
                                , curCandle.timestamp, isCondShort, !isGolden, back_cnt, isBeg, isBreakTrendBottom, isBreakTrendTop, isCrossing, isCrossingSub, fib_short_index, isFibShort), onlyConsole);
                        }
                    }
                ...
                else
                {
                    if (m_position.isLongReserved())
                    {
                        bool isBreakFibOrigin = false;
                        {
                            int fib_origin_over_cnt = 0;
                            for (int i = m_candleBuf.getCandleCount() - 1; i >= 0; i--)
                            {
                                if (fib_origin_over_cnt >= fib_orign_over_limit)
                                {
                                ...
                                if (m_position.fib_origin_high < candle.low)
                                {
                                    fib_origin_over_cnt++;
                                }
                                else
                            ...
                            if (isBreakFibOrigin)
                            {
                                Console.WriteLine("## FibOriginHigh break. fib_origin={0}", m_position.fib_origin_high);
                            }
                        }
                        ...
                        {
                            postSlack(string.Format("{0} Cancel Long Reserved Condition. isCond={1} isGold={2} bkCnt={3} isBeg={4} brkBtm={5} brkTop={6} ingEma={7} ingEmaS={8} isFib({9})={10}"
                                , curCandle.timestamp, isCondLong, isGolden, back_cnt, isBeg, isBreakTrendBottom, isBreakTrendTop, isCrossing, isCrossingSub, fib_long_index, isFibLong), onlyConsole);
                        }
                    }
                    else if (m_position.isShortReserved())
                    {
                        bool isBreakFibOrigin = false;
                        {
                            ...
                            for (int i = m_candleBuf.getCandleCount() - 1; i >= 0; i--)
                            {
                                ...
                                if (m_position.fib_origin_low > candle.high)
                                {
                                    fib_origin_over_cnt++;
                                }
                                else
                            ...
                            if (isBreakFibOrigin)
                            {
                                Console.WriteLine("## FibOriginLow break. fib_origin={0}", m_position.fib_origin_low);
                            }
                        }
                        ...
                        {
                            postSlack(string.Format("{0} Cancel Short Reserved Condition. isCond={1} isDead={2} bkCnt={3} isBeg={4} brkBtm={5} brkTop={6} ingEma={7} ingEmaS={8} isFib({9})={10}"
                                , curCandle.timestamp, isCondShort, !isGolden, back_cnt, isBeg, isBreakTrendBottom, isBreakTrendTop, isCrossing, isCrossingSub, fib_short_index, isFibShort), onlyConsole);
                        }
                    }
                }
        ...
        public bool isConditionShortEntryFL(double next_open, bool onlyConsole = true)
        {
            ...
            try
            {
                ...
                double position = m_frontlineShort - curCandle.last;
      ...
      //              }
        ...
        public bool isConditionShortExitFL(ref int short_lc_cnt, double next_open, bool onlyConsole = true)
        {
            ...
            try
            {
                ...
                if (m_position.entry_fib_idx >= 0)
                {
                    int fib_fwd_index = m_position.entry_fib_idx;
                    if ((fib_fwd_index - 1) < 0)
                    {
                        fib_fwd_index = 0;
                    }
                    else
                    ...
                    double fib_fwd_rate = fib_rates[fib_fwd_index];
                    fib_fwd = high_max - (high_max - low_min) * (1.0 - fib_fwd_rate);
                }
                ...
                else
                {
                    ...
                    //else if (profit >= frontline_ahead2*5.0 || (profit >= frontline_ahead2 && (m_position.fib_origin_low) > curCandle.high))
        ...
        public bool isConditionLongExitFL(ref int long_lc_cnt, double next_open, bool onlyConsole = true)
        {
            ...
            try
            {
                ...
                if (m_position.entry_fib_idx >= 0)
                {
                    ...
                    double fib_fwd_rate = fib_rates[fib_fwd_index];
                    fib_fwd = high_max - (high_max - low_min) * fib_fwd_rate ;
                }
                ...
                if (Math.Abs(m_frontlineLong - m_position.entry_price) <= double.Epsilon)
                {
                    ...
                    else
                    {
                        ...
                        //{
                        //	postSlack(string.Format("## front-line is escape ##. last={0:0} pos={1:0} front={2:0}", curCandle.last, profit, m_frontlineLong), onlyConsole);
                        //	result = true;
                ...
                else
                {
                    ...
                    //else if (profit >= frontline_ahead2*5.0 || (profit >= frontline_ahead2 && m_position.fib_origin_high < curCandle.low))
                    else if (profit >= frontline_ahead2)
                    {
                        // 最前線を前進
